...
if($action == 'update_user_list') {
    $chatUsers = $api->API_whatsappChatUsers($userid);
    $data = array(
...
if($action == 'insert_chat') {
    $api->API_whatsappInsertChat($to_user_id, $userid, $chat_message);
}

if($action == 'show_chat') {
    //$api->API_showUserChat($userid, $to_user_id);
    $user = $api->API_getWhatsappUserDetails($to_user_id);
    //$userSection = '<img src="userpics/'.$user->avatar.'" alt="" />'.
    $userSection = $user->username;

    $userChat = $api->API_getWhatsappUserChat($userid, $to_user_id, $action);

    // Change Unread Messages to Read
    $api->API_editWhatsappUserStatus($userid, $to_user_id, $action);
    $data = array(
...
if($action == 'update_unread_message') {
    $count = $api->API_getWhatsappUnreadMessageCount($to_user_id, $userid);
     // Change Unread Messages to Read
...
if($action == 'update_message_status') {
    // Change Unread Messages to Read
    $api->API_editWhatsappUserStatus($userid, $to_user_id);
}
