...
(function (factory) {
    ...
    $.snackbar = function(options) {
        ...
        if (isset(options) && options === Object(options)) {
            ...
            // Added by chris@goautodial.com
            options.position = isset(options.position) ? options.position : 'left';
            
            if (isset(options.position)) {
                var showLeft = "20px";
                var showRight = "initial";
                if (options.position == "right") {
                    showRight = "20px";
                    showLeft = "initial";
                }
                $("#snackbar-container").css({"left": showLeft, "right": showRight});
				$snackbar.attr("data-position", options.position);
            }
            // END
        ...
        if (!this.hasClass("snackbar")) {
            ...
            if (!isset(action) || action === "show" || action === "hide" || action == "toggle") {
                options = {
                    ...
                    position: $(this).attr("data-potition")
                };
            }
